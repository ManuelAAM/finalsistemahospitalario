...
-- ============================================================
-- FUNDAMENTO LEGAL:
-- La NOM-004-SSA3-2012 establece que el expediente clínico debe:
-- 1. Mantener su integridad (no se pueden eliminar registros)
-- 2. Ser trazable (registrar quién, qué, cuándo)
-- 3. Conservarse por mínimo 5 años
-- 4. Ser confidencial y accesible solo a personal autorizado
--
-- IMPLEMENTACIÓN:
-- Los triggers siguientes IMPIDEN la eliminación física (DELETE) de:
-- - Notas de enfermería (nurse_notes)
-- - Signos vitales (vital_signs)
-- - Tratamientos (treatments)
-- - Historial médico (medical_history)
-- - Tratamientos no farmacológicos (non_pharmacological_treatments)
-- - Reportes de turno (nursing_shift_reports)
--
-- SANCIÓN POR VIOLACIÓN:
-- - Multas económicas según COFEPRIS
-- - Suspensión de licencia profesional
...
-- Trigger: Impedir eliminación de historial médico
CREATE TRIGGER IF NOT EXISTS prevent_delete_medical_history
BEFORE DELETE ON medical_history
BEGIN
  SELECT RAISE(ABORT, 'NOM-004 VIOLACIÓN: No se permite eliminar registros del historial médico. El expediente clínico debe mantener su integridad completa para garantizar trazabilidad legal.');
END;
...
-- Trigger: Impedir eliminación de reportes de turno de enfermería
CREATE TRIGGER IF NOT EXISTS prevent_delete_nursing_shift_reports
BEFORE DELETE ON nursing_shift_reports
BEGIN
...
-- ============================================================
-- Si se requiere "ocultar" registros sin eliminarlos físicamente,
-- se debe usar una columna 'deleted_at' o 'is_active' y filtrar
-- en las consultas SELECT.
--
-- Ejemplo de soft delete:
-- UPDATE nurse_notes SET deleted_at = CURRENT_TIMESTAMP WHERE id = ?;
-- 
-- Consulta excluyendo "eliminados":
-- SELECT * FROM nurse_notes WHERE deleted_at IS NULL;
-- ============================================================
