      ...
      <div style={{ height: totalHeight, position: 'relative' }}>
        <div style={{ transform: `translateY(${offsetY}px)` }}>
          {visibleItems.map((item, index) => {
            const stableKey = item && (item.id || item.key || item._id) ? (item.id || item.key || item._id) : startIndex + index;
            return (
              <div
                key={stableKey}
  ...
  return (
    <div className={className}>
      {items.map((item, index) => (
        <React.Fragment key={item && (item.id || item.key || item._id) ? (item.id || item.key || item._id) : index}>
